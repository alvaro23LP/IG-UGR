...
void cambiarIlum(){ if(ilum) ilum = false; else ilum = true; }
...
Malla malla1("plys/ballIN.ply"), malla2("plys/footbones.ply"); 

//ID OBJETOS SELECIONABLES (1-254)
int idCubo = 1, idCubo2 = 254, idBallIn = 50, idFoot = 100;
...
void dibujoEscena () {
  ...
  float  Negro[4] = { 0.0, 0.0, 0.0, 1 }, Verde[4] = { 0.0, 1.0, 0.1, 1 }, Rojo[4] = { 1, 0.0, 0.0, 1 },
       Blanco[4] = { 1.0, 1.0, 1.0, 1 }, Gris[4] = { 0.4, 0.4, 0.4, 1 }, Azul[4] = { 0.0, 0.0, 1.0, 1 }; 
  ...
  glPushMatrix ();		// Apila la transformacion geometrica actual
    ...
    // Activa o desactiva las texturas si estoy selecionando
    seleccionando ? glDisable(GL_TEXTURE_2D) : glEnable(GL_TEXTURE_2D);

    // OBJETOS SELECCIONABLES
    glPushMatrix();
    if(*eleccion == idBallIn ){ 
        malla1.setAmbient(Azul);
        malla1.setDiffuse(Azul);
    ...
    glPushMatrix();
        if(*eleccion == idFoot ){ 
        malla2.setAmbient(Azul);
        malla2.setDiffuse(Azul);
        malla2.setSpecular(Verde);
        malla2.setShininess(5); //0 - 128
      } else {
        malla2.setAmbient(Blanco);
        malla2.setDiffuse(Gris);
        malla2.setSpecular(Verde);
        malla2.setShininess(5); //0 - 128
      }
      ColorSeleccion(idFoot);
      glTranslated(-20.0f,0.0f,0.0f);
      glScalef(2.0f,2.0f,2.0f);
      suave ? malla2.drawSMOOTH() : malla2.draw(); 
    glPopMatrix();
  
    glMaterialfv(GL_FRONT,GL_SPECULAR, Negro);
    glMaterialf(GL_FRONT, GL_SHININESS, Negro[0]);
    ...
    glPushMatrix();
      ...
      ColorSeleccion(idCubo);
      glTranslated(-2.5f,-2.5f,-2.5f);
      cubo1.asignarTex("dado.jpg");
      cubo1.draw();
    glPopMatrix();
...
void idle (int v)
{
  ...
  glutTimerFunc (300, idle, 0);	// Vuelve a activarse dentro de 30 ms
}
